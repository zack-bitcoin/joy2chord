...
int joy2chord::setup_uinput_device()
{
	...
	// these parts need better error checker, or fix ioctrl_wrapper so it works with them
		...
		if ( -1 == wresult)
    	{
        	cerr << "Unable to write to UINPUT device. Error: " << strerror(errno) << endl;
    	}	
...
*/
	...
	if (!(config.readInto(quickpress, "quickpress")))
	{
...
void joy2chord::write_uinput()
{
    int result = write(uinp_fd, &event, sizeof(event));
    if ( -1 == result)
    {
        cout << "Unable to write to uinput file handler" << endl;
    }
}
...
void joy2chord::process_events2(js_event js)
{
  ...
  //               	case JS_EVENT_BUTTON:
			...
			for (int allbuttons = 0; allbuttons < total_chorded_buttons; allbuttons++)
			{
				...
				{
					button_code += (send_code[allbuttons] * (1<<allbuttons));
				}
			}
